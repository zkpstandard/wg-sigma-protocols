...
\begin{enumerate}
  ...
  \item\label{item:basic:p2}
    ...
    \begin{enumerate}
      ...
      \item
	It then parses $(r_1,\dots,r_n)\defeq \st$ and $(w_1,\dots,w_n)\defeq \witness$.
      \item
        It computes its response as $s_i\defeq r_i+cw_i$ for $i=1,\dots,n$.
    \end{enumerate}
  \item\label{item:basic:v}
    The verifier's algorithm $\verifier(\statement,\commitment,\challenge,s)$ proceeds as follow:
    \begin{enumerate}
      \item It parses $(s_1, \dots, s_n) \defeq s$, $(\commitment_1, \dots, \commitment_m) \defeq T$, and $(Y_1, \dots, Y_m) = Y$.
      \item\label{item:basic:v:checks}
        It checks that $s_i\in\ZZ_p$ for $i=1,\dots,n$ and $\commitment_j\in\GG$ for $j=1,\dots,m$, and outputs $0$ if this is not the case.
      \item
  ...
  \item\label{item:basic:sim}
    ...
    \begin{enumerate}
      ...
      \item
        Finally, the algorithm ouputs the simulated transcript by setting $(\commitment,\challenge,s)\defeq((\commitment_1,\dots,\commitment_m),\challenge,(s_1,\dots,s_n)$.
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
  \item
    ...
    \end{enumerate}
  \item
    The prover's second algorithm $\prover_2(\witness,\statement,\challenge,\st)$ proceeds as follows:
    \begin{enumerate}
      \item
        It checks that $c\in\ZZ_u$ and aborts if this is not the case.
      \item
...
Random vector $\vec a$ can be deterministically generated by fixing $a_0 \defeq 1$ and $(a_1, \dots, a_{\ell-1}) \defeq \prg(\vec Y, \vec T, \vec \challenge, \vec s)$.
...
Nothing prevents those same techniques to be adopted in the (more general) topic of $\Sigma$-protocols.
