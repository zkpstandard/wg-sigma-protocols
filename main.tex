...
\begin{itemize}
  ...
  Browser, and others.
  There are also other works, published in recent academic conferences, that rely on them: Cryptography for \#metoo~\cite{PoPETS:KuyKraRab19}; Solidus~\cite{CCS:CZJKJS17}; ClaimChain~\cite{ClaimChain}.
...
%\href{https://tools.ietf.org/html/rfc2785#ref-LAW}{RFC2785}, that already provides solutions for avoiding small subgroup attacks)
  ...
  \item \textbf{A weak impelmentation of Fiat--Shamir heuristic compromises adaptive security.} The second message in a sigma protocol, sometimes called \emph{challenge}, if computed non-interactively must include not only the commitment, but the full statement and the group description.

  If the random oracle is invoked solely on the commitment (c.f.\ weak Fiat-Shamir transformation~\cite[Def.\ 2]{AC:BerPerWar12}), then it is possible, given a proof $\pi$, to produce another proof $\pi'$ for a different statement without knowing the witness.
...
\begin{enumerate}
  ...
  \item\label{item:basic:v}
    ...
    \begin{enumerate}
      \item It parses $(s_1, \dots, s_n) \defeq s$, $(T_1, \dots, T_m) \defeq T$, and $(Y_1, \dots, Y_m) = Y$.
...
\subsection{Composition of $\Sigma$-Protocols}\label{sec:composition}
  ...
  \minote{Add; This allows for proving statements, possibly across different implementations.}
...
\begin{enumerate}
  \item
    ...
    \begin{enumerate}
      ...
      \item
        It computes $(T^0,\st^0)\gets\prover_1^0(w^0, Y^0)$ and $(T^1,\st^1)\gets\prover_1^1(w^1, Y^1)$.
      \item
...
\jannote{Describe how batching works and why points need to be validated}
...
\begin{enumerate}
  \item
    ...
    \begin{enumerate}
      \item
        The algorithm first computes $(T,\st)\gets\prover_1(Y,w)$.
        \minote{sometimes we use lower-case, sometimes upper-case for indexing?}
      \item
        It computes the challenge by setting $c\defeq\hash(T,Y,\ctx)$.
      \item
        The algorithm defines $s\gets\prover_2(Y,w,c,\st)$.
...
\begin{tabular}{rcccc}
  Project & Language & AND-composition & OR-composition \\
  \hline
  \verb|Emmy| & \\
  %\verb|Helios| \cite{USENIX:Adida08} & \\
  \verb|Kyber| & Go \\
  \verb|SCAPI| \cite{scapi} & \\
  \verb|zkp| \cite{zkp} & Rust\\
  \verb|zksk| \cite{zksk} & Python\\
  \hline
\end{tabular}
\end{table}
