...
In a nutshell, the idea is that the designated verifier provides the prover with a public key, and the verifier then proves that he knows a witness for the claimed relation, or the secret key corresponding to the public key, using the composition techniques from \cref{sec:composition}.
The prover is then unable to forward the proof to a third party, as he could have generated the proof himself using the secret key.
