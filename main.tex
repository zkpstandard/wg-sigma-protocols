...
\begin{enumerate}[label=$(\alph*)$]
 ...
 Consider e.g.\ a prime $p$ that is close to $3/4 \cdot 2^{256}$. Reducing a random $256$-bit integer modulo this $p$ yields a value that is in the range $[0, p / 3]$ with probability roughly $1/2$~\cite{hash-to-curve}.
  ...
  Unfortunately, since the hash function is used also for hashing the initial (secret) randomness used for computing the commitment, this method is undesirable due to the difficulty of implementing a constant-time version (which could lead a secret for the deterministic computation of the commitment).
\end{enumerate}
For the above reasons, we deem the options \cref{item:truncate-128,item:truncate-256} most fit a a real-wold deployment.
...
The case of batched verification must include a point verification sub-routing that asserts the statement and commitments are in question. Failure to properly check that a commitment is in the group could lead to subgroup attacks XX or invalid curve attacks XXXX.
