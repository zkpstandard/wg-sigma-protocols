...
\]
 ...
 \viscontinote{Specify the relation that you want to prove to avoid proving weaker statements.}
\begin{enumerate}
...
\marynote{Why are you multiplying by $(c + 1)$}
...
\begin{remark}
  ...
  \marynote{Incomplete last sentence: “while the first one While the first protocol accepts”}
\end{remark}
...
\marynote{Homomorphism has not been defined}
Let $\GG$ be a group over an elliptic curve with prime order $p$.
  ...
  \item
    ...
    \begin{enumerate}
      \item\label{item:fslong:v:recomputet}
        ...
        \marynote{Very unclear. I think you mean e.g. $T = g^s Y^(-c)$. Maybe refer back to a specific simulator and say which steps, or else just write it out again.}
        \item
...
\begin{theorem}
  ...
  Finally, if $\varphi(\vec x')\ne\varphi(\vec x'')$ for all $\vec x'\ne \vec x''$, the protocol has unique responses.
\end{theorem}
...
\viscontinote{Page 13, th. 8, connection between $u_i$ and $u$ is unclear}
...
For formal proofs we refer, e.g., to Damg\r{a}rd~\cite{damgard04}.
  ...
  \item
    ...
    \minote{Existence of a straight-line extractor for $\Sigma$-protocols was proposed by Fuchsbauer et al.~\cite{C:FucKilLos18}, but under the (stronger) setting of the algebraic group model.}
  \item
...
\marynote{Simulation sound definitions do not exist anymore. Assuming this is an artefact from an older version.}
...
\minote{either or proof I'm you or the staement is true + commitment discrete log}
