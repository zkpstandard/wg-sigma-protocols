...
\begin{abstract}
  ...
  The document provides the necessary formal background, specifies the protocols in full details, provides examples, suggests concrete instantiations (e.g., regarding the selection of elliptic curves or hash functions), and finally sketches a minimal API that should be consistent across implementations.
...
\end{enumerate}
Despite options \cref{item:truncate-256,item:truncate-cfrg} are the most fit for computing the challenge, only option \cref{item:truncate-cfrg} is suitable for computing also the initial nonce. in the non-interactive case. \cref{item:truncate-128} is a valid option for the interactive version.
