...
Introduced by Schnorr~\cite{JC:Schnorr91} over 30 years ago, play an essential component in the building of a number of cryptographic
constructions,
...
\begin{example}{Discrete logarithm equality}{R_DLOG}
  ...
  $$
	  \dleqrelation = \set{\left((\statement_1,\statement_2),\witness)\right)~:~\statement_1=\witness G ~\land~ \statement_2=\witness H}\,.
	$$
\end{example}
...
\begin{example}{Representation}{R_REP}
  ...
  $$
	  \reprelation = \set{\left(\statement,(\witness_1,\witness_2))\right)~:~\statement=\witness_1G + \witness_2H}\,.
	$$
...
In particular we will focus on non-malleable extractable proofs, that is, proofs where the the witness does not only exist but it really must be known by the prover.
...
\begin{figure}
  \begin{tcolorbox}
    \centering
    \procedure{}{
      \textbf{Prover} \< \< \textbf{Verifier} \\[1em]
    \textbf{commitment} \< \sendmessageright*[20em]{\commitment} \< \\
    \< \sendmessageleft*[20em]{\challenge}  \< \textbf{challenge}\\
    \textbf{response} \< \sendmessageright*[20em]{\response} \\
    }
  \end{tcolorbox}
...
These methods should be considered \emph{private} and \emph{should not} exposed externally.
...
A $\sigmaprotocol$ consists of the following methods:
      \begin{itemize}
        ...
        \item $\texttt{yesno} \gets \sigmaprotocol.\verifiertt(\commitment, \challenge, \response)$,
          ...
          Implementing correctly this function is vital security, and will be used for computing the challenge in the Fiat-Shamir transform~\cref{sec:fs}.
          One \emph{must} carefully follow the directives written in the relative instantiations. It is crucial for security that the label includes all data available in the statement, including the parameters and the relation being proven.
      ...
      \end{itemize}
      The final two algorithm describe the \emph{zero-knowledge simulator}.
...
\stnote{maybe only support short form if there is only (at most) one $T$}
...
Relying on a hash function allows us to both compute the challenge and generate securely the commitment.
...
\[
  ...
  \end{aligned}
\]
\paragraph{Seeding the commitment.} The method $\sigmaprotocol.\provercommitment()$ is a randomized function that generates a random element, unique per each execution. The entropy for the commitment be seeded as follows:
\[
    \begin{aligned}
...
This method is fixed for all implementations of $\sigmaprotocol$. Note that the state of the hash function is partially shared between the commitment seed inputs and the challenge computation. Implementors may choose to store the partial hash state before generating the commitment, and re-using it when computing the challenge.
...
\begin{enumerate}
  ...
  \item $c^* \gets \sigmaprotocol.\challengett(\msg, \vec \commitment)$
  \item
...
\end{definition}
\minote{Readers that are not familiar with the notation of gorup morphism should think of $\varphi$ as a linear function from $n$ elements into $m$ elements}
  ...
  \item\label{item:basic:p2}
    ...
    \begin{enumerate}
      ...
      \item
        ...
        \minote{confusing, the c given as argument should probably be of a different font}
