...
7 publications use sigma protocols. All these protocols can be implemented using the API that we propose in section~\cref{sec:composition}.


Below, we propose here an incomplete list of protocols that rely on sigma protocols, sorted by the required functionality:
\begin{itemize}
  ...
  The AND-composition of the dlog relations has been used over a number of protocols,
  ...
  (often referred to as OR composition)
...
A request for standardization has already been posted on the \href{https://community.zkproof.org/t/standardizing-sigma-protocols/471/}{zk-proof community} and received a fair amount of interest.
...
\begin{enumerate}
  ...
  \item
	...
	It parses $\st\defeq(\st^j,c^{1-j},s^{1-j})$.
      \item
        It computes $c^j\defeq c-c^{1-j}\mod m$, and sets $s^j\gets\prover_2^j(w^j,y^j,c^j,\st^j)$.
      \item
        It computes $s^0\gets\prover_2^0(w^0,y^0,c,\st^0)$ and $s^1\gets\prover_2^1(w^1,y^1,c,\st^1)$.
      \item
  ...
  \item
    ...
    \begin{enumerate}
      \item
        It  parses $(s^0,s^1,c^0)\defeq$.
      \item
        It sets $c^1\defeq c-c^0\mod m$.
      \item
  ...
  \item
    ...
    \begin{enumerate}
      ...
      \item
        It chooses a random $c^0$ in $\ZZ_p$ and computes $c^1\defeq c-c^0 \mod m$.
      \item
...
Concretely, we see fir standardized curves such as curve p-521, p-256~\cite{XXX}, BITCOIN CURVE~\cite{XX}, or standardized prime-order group abstractions over non prime-order groups, such as ristretto XXX.

We think a modern standard should also take into account pairing-friendly curves, such as XXX, under relevant assumptions for bilinear groups, such as SXDH
\minote{how seriously should we consider safe curves and replaces the curves I suggest?}
We note that the exists already standards for removing small cofactors in elliptic curves~\cite{XX}, howeve we believe that this should be implemented as an abstraction layer over the protocol.

\minote{chek if standards works well}

\paragraph{Commitment.} The first step is the generation of a uniformly-distributed random element over $\ZZ_p$. As mentioned in \Cref{sec:motivation}, the nonce must be distributed uniformly at random, and even a small bias in the distribution could completely compromise zero-knowledge.
...
We propose the generation of a synthetic nonce obtained from hashing the statement, the context, and the secret together, as already suggested in previous standards \href{https://tools.ietf.org/html/rfc6979}{RFC6979}.
While it is widely recognized that this approach leads to the generation of secure nonces, it might on the other hand leak information about the witness used for a proof, when examining consecutive executions of the protocol. This is e.g. the case of a OR-proof that attempts to preserve anonymity.
 For these cases, the prover can either rely on an internal counter, or simply rely on the operating system's random number generator.
...
The hash function can be either blake2 or sha256 truncated to the first 128 buts, or a more structured hash into a prime $p$. For this, we direct the reader to XX.
...
\minote{mention concatenation could lead to attacks. zkp uses strobe to get aroudn it.}
...
\minote{R1CS compatibility.\\ a unified way of specifying proof goals}
