...
\begin{remark}{Equivalent statements}{}
  ...
  Using the above notation, we have $\varphi: (w_0,w_1)\mapsto (w_0G,w_1H)$.
...
Yet, the mapping $\FF_p^2\to\GG^3:(w_1,w_2)\mapsto (w_1G,w_2G,w_1w_2G)$ is not a homomorphism, and consequently the basic protocol presented before cannot be deployed directly.
...
\begin{itemize}
  \item  $\varphi:\FF_p^2\to\GG^3:(w_1,w_2)\mapsto(w_1G,w_2G,w_2\statement_1)$
...
\begin{itemize}
  \item $\varphi:\FF_p^m\to\GG:(w_1,w_2, \dots, w_m)\mapsto \sum_i w_iG_i$
  \item $\rep.\morphismlabel()$ returns
  \[
    \hash(\replabel \concat \curvectx \concat \serialize(G_1) \concat \cdots \concat \serialize(G_m) \concat \serialize(Y))
  \]
...
\[
   \commitment_i + c_i Y_i = \sum_j s_j G_{i,j}
\]
for $i=1, \dots,\ell$,
...
\[
  \left(\sum_{i=1}^{\ell} a_i \commitment_i\right) + \left(\sum_{i=1}^{\ell} a_i \cdot  c_i \statement_i \right) = \left(\sum_{i=1}^{\ell} a_i \cdot \sum_j \response_j G_{i,j}\right).
\]
...
\[
  \left(\sum_{i=1}^{\ell} a_i \commitment_i\right) + \left(\sum_{i=1}^{\ell} (a_i \cdot  c_i) \statement_i \right) + \left(\sum_{i=1}^{\ell}\sum_j (-a_i \cdot  \response_j) G_{i,j}\right) = 0.
\]
