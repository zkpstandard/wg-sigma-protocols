...
A basic $\Sigma$-protocol for the relation $\relation=\{((y_1,\dots,y_m),(w_1,\dots,w_n)) : (y_1,\dots,y_m)=\varphi(w_1,\dots,w_n)\}$ for a group homomorphism $\varphi:\ZZ_p^n\to\GG^m$ is given by the following algorithms:
...
\begin{enumerate}
  ...
  \item
    ...
    \begin{enumerate}
      ...
      \item
	It checks whether $(t_1 + cy_1,\dots,t_m + cy_m) = \varphi(s_1,\dots,s_n)$, and outputs $1$ if this is the case; otherwise, $\verifier$ outputs $0$.
    \end{enumerate}
  \item
...
Let $\GG$ be a group over an elliptic curve with prime order $p$.
Proving equality of the known discrete logarithm $w$ of $Y_1$ in base $G$ and $Y_2$ in base $H$ means proving knowledge of $w\in\ZZ_p$ such that $Y_1=wG$ and $Y_2=wH$.
