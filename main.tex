...
\stnote{the way it is phrased now is perfect. I wouldn't discuss this (yet) - most protocols (standard protocol, AND composition) for reasonable relations have this perfectly anyways... not sure about OR composition though}
  ...
  \item
	...
	It parses $\st\defeq(\st^j,c^{1-j},s^{1-j})$.
      ...
      \item
        ...
        \stnote{Why? The partial challenge (for one of the subprotocols) needs to be included to enable verification, so it should be $c$... ?}
    \end{enumerate}
  \item
    The verifier's algorithm $\verifier(\vec \statement, \vec \commitment,\challenge,\vec \response)$ proceeds as follows:
    \begin{enumerate}
...
The random vector $\vec a$ can be \emph{deterministically} generated by fixing $a_0 \defeq 1$ and setting $(a_1, \dots, a_{\ell-1}) \defeq \prg(\vec Y, \vec T,  \challenge, \vec s)$.
...
Damg\r{a}rd~\cite{EC:Damgard00}
\stnote{stephan: add intro}
\begin{enumerate}
  \item
    The verifier's first algorithm $\verifier_1(y)$ proceeds as follows:
    \begin{enumerate}
      \item
	It samples $c\sample\ZZ_p$ and computes $(com,o)\defeq{\sf Commit}(ck,c)$.
      \item
        It sets $\st_\verifier\defeq(c,o)$.
      \item
        It sends $com$ to the prover.
    \end{enumerate}
  \item
    The prover's first algorithm $\prover_1(w,y,com)$ consists of the following steps:
    \begin{enumerate}
      \item
	The algorithm computes $(t',\st') \defeq \prover_1(w,y)$.
      \item
	It outputs $(t,\st) \defeq (t',(\st',com))$.
    \end{enumerate}
  \item
    The verifier's second algorithm $\verifier_1(y,t,\st_\verifier)$ proceeds as follows:
    \begin{enumerate}
      \item
        It parses $\st_\verifier=(c,o)$.
      \item
        It sends $(c,o)$ to the prover.
    \end{enumerate}
  \item
    The prover's second algorithm $\prover_2(w,y,c,o,\st)$ proceeds as follows:
    \begin{enumerate}
      \item
        The algorithm parses $\st=(\st',com)$.
      \item
        It checks that ${\sf VerCommit}(com,c,o)=\accept$ and aborts otherwise.
      \item
        It outputs $s\defeq\prover_2(w,y,c,\st')$.
    \end{enumerate}
  \item
    The verifier's final algorithm $\verifier_3(y,t,c,s)$ proceeds as follow:
    \begin{enumerate}
      \item
	The algorithm outputs $\verifier(y,t,c,s)$.
    \end{enumerate}
\end{enumerate}
...
We think pairing-friendly curves, such as BLS12-381~\cite{bls12}, should also be considered for inclusion under relevant assumptions for bilinear groups, such as XDH when dealing with DH triples.
 ...
 \paragraph{Choice of the hash funcitons.} We advise for the use of consolidated hash functions such as \verb|SHA2|, \verb|SHA3|, and \verb|BLAKE2|, \verb|BLAKE3|.
 We did not examine closely algebraic hashes for this draft, and velieve this must include much deeper cryptanalitic analysis within a working group.
