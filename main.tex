...
\begin{figure}[t]
  \begin{tcolorbox}
    \centering
    \procedure{}{
      \textbf{Prover} \< \< \textbf{Verifier} \\[][\hline]\\
...
\begin{itemize}
  ...
  $\orcomposition.\verifiertt(\vec \commitment, \challenge, \vec \response)$,
    \begin{enumerate}
      ...
      \item
	    Return $\pctrue$ if both $\leftbranch.\verifiertt(\commitment_0,\challenge_0,\response_0)$ and $\rightbranch.\verifiertt_1(\commitment_1,\challenge_1,\response_1)$ return $\pctrue$. Otherwise, return $\pcfalse$.
    \end{enumerate}
...
\[
  \relation=\set{((\statement_1,\dots,\statement_m),(\witness_1,\dots,\witness_n)) \in \GG^m \times \FF_p^n: (\statement_1,\dots,\statement_m)=\varphi(\witness_1,\dots,\witness_n)}
\]
...
\begin{itemize}
  ...
  \[
    \hash(\schnorrlabel \concatblock \curvectx \concatblock \serialize(G) \concat \serialize(\statement))
  \]
\end{itemize}
...
\begin{itemize}
  ...
  \[
    \hash(\dleqlabel \concatblock \curvectx \concatblock \serialize(G) \concat \serialize(H) \concat \serialize(\statement_1) \concat \serialize(\statement_2))
  \]
\end{itemize}
