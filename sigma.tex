...
\fancypagestyle{plain}{
  ...
  \fancyhead[R]{\includegraphics[height=0.75cm]{assets/zkproof.png}}% Right header
...
\begin{titlepage}
   \begin{center}
       ...
       \vspace{1.0cm}
			 \textbf{Contributors:}\\[.3em]
       \begin{tabular}{c@{\hspace{5em}}c@{\hspace{5em}}c}
        Jan Bobolz & Mary Maller & Yuwen Zhang \\
       \end{tabular}
       \vfill
       Version: \version\\
       \vspace{0.5cm}
       Date: \today\\
       \vspace{1.3cm}
       \includegraphics[width=0.4\textwidth]{assets/zkproof.png}
   \end{center}
\end{titlepage}
...
\vfill
\begin{center}
  \includegraphics[width=.9\textwidth]{assets/Dinner_Party.png}
\end{center}
...
\begin{figure}[t]
  ...
  \end{tcolorbox}
  \caption{Overview of the steps composing a generic $\Sigma$-protocol. Underlined, the secrets in the the protocol. In practice, the challenge is computed deterministically and the protocol is non-interactive. See \cref{sec:fs} for more information.}
\end{figure}
...
We define a template class for $\Sigma$-protocols denoted $\sigmaprotocol$. This is the basic interface that will be implemented remainder of this document.
...
A $\sigmaprotocol$ consists of the following methods:
      \begin{itemize}
        ...
        \item $\texttt{yesno} \gets \sigmaprotocol.\verifiertt(\commitment, \challenge, \response)$,
          ...
          Implementing this function correctly is vital security, and it must include all data available in the statement, as well as the parameters and the relation being proven. The label will be used for computing the challenge in the Fiat-Shamir transform~\cref{sec:fs}.
...
Te mapping $\FF_p^2\to\GG^3:(\witness_1,\witness_2)\mapsto (\witness_1G,\witness_2G,\witness_1\witness_2G)$ is not a homomorphism, and consequently the basic protocol presented before cannot be deployed directly.
